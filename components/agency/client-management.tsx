      ...
      <div className="bg-white/10 backdrop-blur-md rounded-lg border border-white/20 p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-white">Airtable Client Mapping</h2>
          </div>
          <button
            onClick={async () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {clients.filter(c => c.is_active).map(client => (
            <div key={client.id} className="p-4 border border-white/20 rounded-lg bg-white/5">
              <div className="flex items-center gap-2">
                <Building2 className="h-4 w-4 text-white/70" />
                <div>
                  <div className="text-sm font-medium text-white">{client.brand_name}</div>
                  <div className="text-xs text-white/50">→ {client.brand_slug}</div>
                </div>
              </div>
            </div>
          ))}
